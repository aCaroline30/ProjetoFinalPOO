...
public class SistemaDeRecomendacao {
    ...
    public List<Filme> recomendarPara(Cliente cliente) {
        if (cliente.getHistoricoFilmesIds().size() < 3 && cliente.getPreferenciaGeneros().isEmpty()) {
            return Collections.emptyList();
        }

        Set<String> generos = new HashSet<>(cliente.getPreferenciaGeneros());
        for (String filmeId : cliente.getHistoricoFilmesIds()) {
        ...
        return gerenciadorDeFilmes.listarTodos().stream()
                .filter(f -> f.getGenero() != null && generos.contains(f.getGenero()) && !cliente.getHistoricoFilmesIds().contains(f.getId()))
